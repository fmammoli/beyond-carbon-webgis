...
DEFAULT_BASE_URL = env_value(
    "LANDCOVER_API_BASE_URL",
    "LANDCOVER_STATS_LOCAL_API_URL",
    default="http://127.0.0.1:8000",
)
DEFAULT_API_KEY = env_value(
    "LANDCOVER_STATS_API_KEY",
    "CHM_API_KEY",
    "API_KEY",
)
...
def build_jobs_base_url(base_url: str) -> str:
    ...
    if normalized_path.endswith("/api/v1/landcover/stats/jobs"):
        jobs_path = normalized_path
    elif normalized_path.endswith("/api/v1/landcover/stats"):
        jobs_path = f"{normalized_path}/jobs"
    elif normalized_path.endswith("/jobs"):
        jobs_path = normalized_path
    else:
        jobs_path = f"{normalized_path}/api/v1/landcover/stats/jobs" if normalized_path else "/api/v1/landcover/stats/jobs"

    return urlunsplit((parsed.scheme, parsed.netloc, jobs_path, parsed.query, parsed.fragment)).rstrip("/")


def create_job(base_url: str, api_key: str) -> dict[str, Any]:
    url = build_jobs_base_url(base_url)
    payload = {
...
def poll_job(base_url: str, api_key: str, job_id: str, *, timeout_seconds: int = 600, poll_interval: float = 2.0) -> dict[str, Any]:
    url = f"{build_jobs_base_url(base_url)}/{job_id}"
...
def main() -> int:
    ...
    if not args.api_key:
        print("Missing API key. Provide --api-key or set LANDCOVER_STATS_API_KEY/CHM_API_KEY/API_KEY.", file=sys.stderr)
        return 2
